...
if "messages" in st.session_state:
    for msg in st.session_state.messages.debate_history:
        st.chat_message(name=msg["role"], avatar=msg["avatar"]).write(msg["content"])



def debate_round():
    for expert in st.session_state["experts"]:
        with st.chat_message(name=expert.expert_instruction["role"], avatar=expert.expert_instruction["avatar"]):
            response = expert.generate_argument(st.session_state.messages, StreamHandler(st.empty()))
            st.session_state.messages.add_message(role=expert.expert_instruction["role"], avatar=expert.expert_instruction["avatar"], content=response)
...
if submitted and topic.strip() != "":
    ...
    st.session_state["experts"] = st.session_state["worker_manager"].create_experts(num_experts=number_experts, topic=topic)
...
if "initialized" in st.session_state and st.session_state["initialized"]:
    ...
    st.button("Next round of debate")
